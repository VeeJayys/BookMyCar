...
public class DriverServiceImpl implements DriverService{
    ...
    public String saveDriver(DriverRequest driverRequest) {
        ...
        if(driverRepo.save(driver)!=null) return "Driver saved";
        return "Driver didnt save, please try again";
    }

    @Override
    public List<DriverResponse> getAllDrivers() {
        return driverRepo.findAll().stream().filter(x->x.getStatus().equals("available")).map(e->modelMapper.map(e, DriverResponse.class)).collect(Collectors.toList());
    }
